    ...
    >
      {cornerImages.map((img, i) => (
        <FadeIn key={i} {...img.fadeProps} className={img.className}>
          <img src={img.src} alt={img.alt} className="w-full select-none pointer-events-none" draggable={false} />
        </FadeIn>
      ))}
      ...
      <div className="flex flex-col items-center gap-10 sm:gap-14 md:gap-16 z-10">
        ...
        <div className="flex justify-center w-full px-4" style={{ fontSize: 'clamp(1rem, 2vw, 1.35rem)' }}>
          <AnimatedText
